    ...
    class Network_VRIK : MonoBehaviour_Network
    {
        ...
        private void Start()
        {
            ...
            if (playerNetwork)
            {
                ...
                headIK = new GameObject("Head IK Target").transform;
                headIK.parent = transform;
                headIK.ResetTransform();

                leftHandIK = new GameObject("Left Hand IK Target").transform;
                leftHandIK.parent = transform;
                leftHandIK.ResetTransform();

                rightHandIK = new GameObject("Right Hand IK Target").transform;
        ...
        protected override void OnDestroy()
        {
            ...
            playerDictionary.Remove(playerNetwork);
            base.OnDestroy();
        }

        internal static void UnloadAll()
        {
            List<Network_VRIK> vrikList = playerDictionary.Values.ToList();

            foreach (Network_VRIK netVRIK in vrikList)
            {
                if (netVRIK.initialized)
                {
                    Destroy(netVRIK.vrik);
                    netVRIK.playerNetwork.Animator.anim.runtimeAnimatorController = netVRIK.playerNetwork.currentModel.thirdPersonController;
                }
                Destroy(netVRIK);
            }
        }
        ...
        private void Initialize(float armScale, float legScale, bool leftHanded)
        {
            SetupVRIK(armScale, legScale);
        ...
        private void SetupVRIK(float armScale, float legScale)
        {
        ...
        private void SetupAnimationController()
        {
            ...
            List<AnimationClip> vrOverrideClips = RaftNonVR.clips.ToList();
